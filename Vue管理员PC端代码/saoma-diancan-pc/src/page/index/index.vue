...
export default {
		...
		created() {
			let nuvmenuid = localStorage.getItem("nuvmenuid")
			if (nuvmenuid) {
				this.activeIndex = JSON.parse(nuvmenuid)
			}
			if (this.$route.query && this.$route.query.item) {
				this.activeIndex = this.$route.query.item
			}
		},
		watch: {
			$route(to, from) {
				if (to.query && to.query.item) {
					this.activeIndex = to.query.item
					localStorage.setItem("nuvmenuid", JSON.stringify(to.query.item))
				}
			},
