<template>
	<div class="ordering">
		...
		<div class="heading">桌位管理</div>
		<div class="content-view">
			<div class="button-view">
				<div class="action-section">
					<el-button type="success" size="small" @click="batchUpdateState(0)" :disabled="selectedIds.length === 0">
						<i class="el-icon-check"></i> 批量设为空闲
					</el-button>
					<el-button type="warning" size="small" @click="batchUpdateState(1)" :disabled="selectedIds.length === 0">
						<i class="el-icon-close"></i> 批量设为占用
					</el-button>
					<el-button type="primary" size="small" @click="loadTableList()">刷新</el-button>
				</div>
			</div>
			<div v-if="nodatas" class="table-wrapper">
				...
				<div class="tab-table" v-for="(item,index) in tabcont" :key="index">
					...
					<div class="table-col">{{item.number}}</div>
					<div class="table-col">
					...
					<div class="table-col">
						<el-button-group>
							<el-button size="small" type="success" @click="updateState(item, 0)" :disabled="item.state === 0" :loading="loadState[index] === 0">空闲</el-button>
							<el-button size="small" type="warning" @click="updateState(item, 1)" :disabled="item.state === 1 || item.state === 2" :loading="loadState[index] === 1">占用</el-button>
						</el-button-group>
					</div>
				</div>
				<el-pagination
			...
			<div class="nodatas" v-if="!nodatas">还没有桌位</div>
		</div>
...
export default{
	...
	methods:{
		...
		async loadTableList(){
			try{
				let res = await new this.Request(this.Urls.m().getqrcode,{"pageNum":this.pagenum+1,"pageSize":100}).modepost()
				this.nodatas = res.data.total == 0 ? false : true
				this.tabcont = (res.data.tableList || []).map(item => {
					return {
						...item,
						selected: false
					}
		...
		async updateState(row, state) {
			const index = this.tabcont.findIndex(item => item.id === row.id)
			this.$set(this.loadState, index, state)
			this.$confirm(`确定将桌台 ${row.number} 状态更改为"${this.getStateText(state)}"吗？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(async () => {
				try {
					let res = await new this.Request(`${this.Urls.m().updateState}?id=${row.id}&state=${state}`).modeget()
					...
					this.$set(this.loadState, index, null)
				}
			}).catch(() => {
				this.$set(this.loadState, index, null)
			})
		...
		async batchUpdateState(state) {
			this.$confirm(`确定将选中的 ${this.selectedIds.length} 个桌台状态更改为"${this.getStateText(state)}"吗？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(async () => {
				try {
					...
					for (let id of this.selectedIds) {
						try {
							let res = await new this.Request(`${this.Urls.m().updateState}?id=${id}&state=${state}`).modeget()
							if(res.status === 200 && res.data.code === 0){
								const index = this.tabcont.findIndex(item => item.id === id)
								if (index !== -1) {
									this.$set(this.tabcont[index], 'state', state)
								}
								successCount++
					...
					if (failCount === 0) {
						new this.mytitle(this.$message,'success',`批量更新成功，共 ${successCount} 个桌台`).funtitle()
					} else {
						new this.mytitle(this.$message,'warning',`批量更新完成：成功 ${successCount} 个，失败 ${failCount} 个`).funtitle()
					}
...
.button-view .el-button--primary {
	background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);
	box-shadow: 0 4px 12px rgba(37, 99, 235, 0.3);
}

.button-view .el-button--primary:hover {
